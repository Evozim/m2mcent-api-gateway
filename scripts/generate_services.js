...
const domains = [
  "defi", "healthcare", "ai-safety", "supply-chain", "genomics",
  "logistics", "fintech", "cybersecurity", "web3", "metaverse",
  "iot", "robotics", "nlp", "computer-vision", "data-analytics",
  "climate-tech", "agritech", "edtech", "legaltech", "proptech"
];

const actions = [
  "analyzer", "optimizer", "auditor", "generator", "indexer",
  "predictor", "validator", "extractor", "classifier", "synthesizer",
  "monitor", "simulator", "translator", "router", "aggregator"
];

const targets = [
  "smart-contract", "yield", "trading-bot", "metadata", "zk-proof",
  "genomic-sequence", "carbon-credit", "supply-route", "fraud-detection", "sentiment",
  "image-recognition", "voice-synthesis", "legal-contract", "malware", "biometric"
];

const inputs = ["data", "prompt", "parameters", "query", "payload", "dataset", "image", "audio", "text", "hash"];
const outputs = ["report", "result", "analysis", "prediction", "status", "score", "summary", "hash", "url", "boolean"];

const generateRandomPrice = () => {
  // Generate a price between 0.01 and 1.50 USDC
  const price = (Math.random() * 1.49 + 0.01).toFixed(6);
  return price;
};
...
const generateUniqueId = (i) => {
  ...
  return `${domain}-${target}-${action}-mcp-${i}`;
};

const services = [];

// Always include the original 10 for consistency
services.push({
...
for (let i = 11; i <= 1000; i++) {
  ...
  services.push({
    id: id,
    title: `Execute ${id}`,
    description: `Premium agentic endpoint for ${id.replace(/-/g, ' ')}.`,
    path: `/${id}/api/execute`,
    price: generateRandomPrice(),
    inputProperty: getRandomElement(inputs),
    outputProperty: getRandomElement(outputs)
  });
}
